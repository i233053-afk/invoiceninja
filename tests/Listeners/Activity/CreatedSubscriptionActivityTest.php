...
class CreatedSubscriptionActivityTest extends TestCase
{
    ...
    public function it_saves_activity_with_user_id_from_event_vars()
    {
        ...
        $activityRepoMock->shouldReceive('save')
            ->once()
            ->withArgs(function ($fields, $subscription, $event_vars) {
                $this->assertEquals(1, $fields->subscription_id);
                $this->assertEquals(123, $fields->user_id);
                $this->assertEquals(10, $fields->company_id);
                $this->assertEquals(Activity::CREATE_SUBSCRIPTION, $fields->activity_type_id);
                return true;
            });

        $listener = new CreatedSubscriptionActivity($activityRepoMock);
    ...
    /** @test */
    public function it_saves_activity_with_user_id_from_subscription_when_not_in_event_vars()
    {
        $multiDbMock = Mockery::mock('alias:App\Libraries\MultiDB');
        $multiDbMock->shouldReceive('setDb')->once()->with('test_db');
