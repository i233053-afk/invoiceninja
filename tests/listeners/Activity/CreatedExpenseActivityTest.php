...
class CreatedExpenseActivityTest extends TestCase
{
    ...
    public function it_saves_activity_with_user_id_from_event_vars()
    {
        ...
        $activityRepoMock->shouldReceive('save')
            ->once()
            ->withArgs(function ($fields, $expense, $event_vars) {
                $this->assertEquals(1, $fields->expense_id);
                $this->assertEquals(123, $fields->user_id);
                $this->assertEquals(10, $fields->company_id);
                $this->assertEquals(100, $fields->account_id);
                $this->assertEquals(Activity::CREATE_EXPENSE, $fields->activity_type_id);
                $this->assertNull($fields->recurring_expense_id);
                return true;
            });

        $listener = new CreatedExpenseActivity($activityRepoMock);
    ...
    /** @test */
    public function it_saves_activity_with_user_id_from_expense_when_not_in_event_vars()
    {
        $multiDbMock = Mockery::mock('alias:App\Libraries\MultiDB');
        $multiDbMock->shouldReceive('setDb')->once()->with('test_db');
