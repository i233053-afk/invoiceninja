...
public function findUserNotificationTypes($invitation, $company_user, $entity_name, $required_permissions): array
{
    ...
    if (
        $company_user->company->is_disabled ||
    ...
    // Prevent crash: invitation may not contain this entity (NULL)
    if (
        !isset($invitation->{$entity_name}) ||
        !$invitation->{$entity_name}
    ) {
        // No invoice/quote/etc. attached = no notifications
        return [];
    }

    $entity = $invitation->{$entity_name};

    // If the user owns or is assigned, they keep special permissions
    if (
        $entity->user_id == $company_user->user_id ||
        $entity->assigned_user_id == $company_user->user_id
    ) {
        // do nothing
    } else {
        // Remove special permissions for unrelated users
